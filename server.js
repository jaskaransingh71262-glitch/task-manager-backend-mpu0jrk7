...
app.get('/api/tasks/completed', auth.verifyToken, async (req, res) => {
    try {
        const { id } = req.user;
...
app.get('/api/tasks/incomplete', auth.verifyToken, async (req, res) => {
    try {
        const { id } = req.user;
...
app.put('/api/tasks/:id', auth.verifyToken, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('task_manager_tasks').update({ id: id, title: title, description: description, completed: completed });
        if (error) {
            return res.status(400).json({ error: error.message });
        }
        res.json(data[0]);
...
app.delete('/api/tasks/:id', auth.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { data, error } = await supabase.from('task_manager_tasks').delete().eq('id', id);
        if (error) {
            return res.status(400).json({ error: error.message });
        }
